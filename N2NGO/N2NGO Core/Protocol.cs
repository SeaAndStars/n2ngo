...
public class Protocol
{
    ...
    public readonly static System.Text.Encoding StringEncoding = System.Text.Encoding.Unicode;
    ...
    public enum BaseHeader : byte
    {
        ...
        _rooms_create,                      // Client requests to create room, appends with package{ RoomCode(msg_string), RoomName(msg_string), IsRoomInvisible(msg_byte), IsRoomPasswordNeeded(msg_byte), RoomPassword(msg_string), MainColor(msg_ulong), MinorColor(msg_ulong), InitialLifetime_MilliSeconds(msg_ulonglong) }, server responds package{CB.AdminKey(msg_string_long), msg_ok}.
