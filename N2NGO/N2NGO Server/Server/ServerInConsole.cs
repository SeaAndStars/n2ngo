...
internal class ServerInConsole
{
    ...
    public ServerInConsole(ConsoleBuffer buffer, EasyConfig config)
    {
        if (buffer == null)
        ...
        if (ServerConfig.Get("EnableIPv6", "1") != "0")
        {
            ...
            Server = new Base.N2NGOServer(buffer, ipAddrV4, portV4, ipAddrV6, portV6);
        ...
        {
            Server = new Base.N2NGOServer(buffer, ipAddrV4, portV4);

            Server.ConsoleBuffer.AppendFormatBuffer(ConsoleBuffer.BufferContentType.Info, "Server Config Loaded: Listen {0}:{1}\n", ipAddrV4.ToString(), portV4);
        }
    }

    public ServerInConsole(ConsoleBuffer buffer, string configFilePath) : this(buffer, new EasyConfig(configFilePath)) { }
