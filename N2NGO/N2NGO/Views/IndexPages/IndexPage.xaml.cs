...
public partial class IndexPage : BaseIndexPage
{
    ...
    {
        try { CurrentServerGlobalAddress.Text = $"{(Globals.CurrentApp.N2NGOServerConnection.Client.Client.RemoteEndPoint ?? throw new NullReferenceException(nameof(Globals.CurrentApp.N2NGOServerConnection.Client.Client.RemoteEndPoint))) as IPEndPoint}"; }
        catch (Exception) { }
        CurrentServerSupernodeAddress.Text = $"{Globals.CurrentApp.N2NGOServerConnection.ServerIPEndPoint.Address}:{Globals.CurrentApp.N2NGOServerConnection.ServerSupernodePort}";
        Task.Run(() =>
        {
            var connected = Globals.CurrentApp.N2NGOServerConnection.IsConnected();
            ...
            if (!Globals.CurrentApp.N2NGOServerConnection.Peek(out var latency))
                return;
            Dispatcher.Invoke(() => CurrentServerLatency.Text = $"{latency.Milliseconds}ms");

            var onlines = Globals.CurrentApp.N2NGOServerConnection.PullTotalOnlines();
    ...
    private async void DispatcherTimerCurrentRoomIndexerUpdater_Tick(object? sender, EventArgs e)
    {
        ...
        var _currentCode = Globals.CurrentApp.RoomConnection.CurrentRoomCode;
        if (_currentCode != string.Empty)
        {
            var roomPullResult = await Task.Run(() => Globals.CurrentApp.N2NGOServerConnection.GetRoomByCode(_currentCode));
        ...
        Globals.CurrentApp.MainWindow.PageRoom.UpdateRoomInfoWithLocal(room);
    }

    private void DispatcherTimerCurrentUserIndexerUpdater_Tick(object? sender, EventArgs e)
    {
        CurrentUserNickname.Text = Globals.CurrentApp.Config.Get("UserNickname");
    }
    ...
    protected override void Indexer_MouseLeftButtonUp(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        ...
        switch (indexer.NagivKey)
        {
            ...
            case "SetUserNickname":
                {
                    Globals.CurrentApp.MainWindow.DoMessageInputDialog("@LOCALE_DialogEditNickname_Content", "@LOCALE_DialogEditNickname_Title"
                ...
                (_)=> {
                    ...
                    Globals.CurrentApp.Config.Set("UserNickname",  dialogInput.InputBox.Text);
                } },
                        ...
                        (_) =>
                        {
                            ...
                            dialogInput.InputBox.Text = Globals.CurrentApp.Config.Get("UserNickname");
            ...
            case "ResetConnection":
                {
                    Globals.CurrentApp.MainWindow.DoMessageYesNoDialog("@LOCALE_DialogResetConnectionDialog_Content", "@LOCALE_DialogResetConnectionDialog_Title",
                    new()
                    {
                        (_) =>
                        {
            ...
            case "EnterRoom":
                {
                    if (!Globals.CurrentApp.RoomConnection.IsConnected)
                    {
                        Globals.CurrentApp.MainWindow.DoMessageDialog("@LOCALE_DialogEnterRoom_Fail_Not_Connected_Content", "@LOCALE_DialogEnterRoom_Title");
                        break;
                    }
                    NavigatePage(Globals.CurrentApp.MainWindow.PageRoom);
                    break;
                }
    ...
    {
        Globals.CurrentApp.MainWindow.DoMessageYesNoDialog("@LOCALE_DialogViewUserKey_Description_Content", "@LOCALE_DialogViewUserKey_Title",
                    new()
                    {
                        (_) =>
                        {
                            ...
                            if (dialogYesNo.YesNo == DialogYesNo.YesNoE.Yes)
                                Task.Run(()=>
                                {
                                    var usrKeyResult = Globals.CurrentApp.N2NGOServerConnection.GetUserKey();
                                    if (usrKeyResult.IsSuccessfulStatusCode)
                                    {
                                        if (usrKeyResult.Value is not null)
                                            Dispatcher.Invoke(()=>Globals.CurrentApp.MainWindow.DoMessageDialog(usrKeyResult.Value, "@LOCALE_DialogViewUserKey_Title"));
                                    }
                                });
                        }
                    }
